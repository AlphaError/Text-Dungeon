...
class CharacterCreationViewController: UIViewController {
    ...
    @IBAction func magePress(sender: AnyObject) {
        characterP1 = Character(characterNm: namebox.text!, characterStats: classType[0])  //if button pressed, creates existant class depending on classType
        character = "@  "
    }
    
    @IBAction func swordsmanPress(sender: AnyObject) {
        characterP1 = Character(characterNm: namebox.text!, characterStats: classType[2])
        character = "%  "
    }
    
    @IBAction func knightPress(sender: AnyObject) {
        characterP1 = Character(characterNm: namebox.text!, characterStats: classType[1])
        character = "&  "
    }
